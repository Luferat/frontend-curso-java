...
// Exibe uma mensagem avisando sobre popups bloqueados.
function blockedPopUp(seconds) {

    // Testa suporte a popups.
    var w = window.open('', 'popupTest', 'height=1,width=1,top=10000,left=10000')
    if (!w) {

        $('body').prepend(`
            <div id="blocked">
                <style>
                    .bl-modal { position: fixed; top: 0; left: 0; width: 100%; z-index: 110; background-color: rgba(0,0,0,.5); display: flex; align-items: center; justify-content: center; padding: 2rem; }
                    .bl-body { background: #ffc; border-radius: .3rem; padding: .5rem; width: 100%; max-width: 400px; margin: auto; font-size: 80%; }
                    .bl-close { float: right; margin-right: -.25rem; }
                    .bl-body img { max-width: 90%; height: auto; }
                    .bl-body h3 { margin: .25rem 0; }
                    .bl-footer { text-align: center; padding: .5rem .5rem 0 .5rem; }
                </style>
                <div class="bl-modal">
                    <div class="bl-body">
                        <div class="bl-close"><i class="fa-solid fa-xmark fa-fw"></i></div>
                        <img src="img/blocked.png" alt="Popups bloqueados">
                        <h3>Oooops!</h3>
                        Os popups estão bloqueados no seu navegador. Por favor, permita os popups do navegador para este site funcionar melhor.
                        <div class="bl-footer"><a href="policies">Políticas de Privacidade</a></div>
                    </div>
                </div>
            </div>    
        `)
        ...
        // Eventos do popup.
        $('.bl-modal').click(() => { $('#blocked').remove() })
        setTimeout(() => { $('#blocked').remove() }, seconds * 1000 || 5000)
    } else {
        w.close()
    }
